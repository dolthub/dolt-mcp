...
func TestMutationToolsAnnotations(t *testing.T) {
	cases := []struct {
		...
		{"dolt_reset_soft", NewDoltResetSoftTool, false, false, true, false},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			tool := tc.ctor()
			ann := tool.Annotations
			if b(ann.ReadOnlyHint) != tc.ro {
				t.Fatalf("%s: readOnly got %v", tc.name, b(ann.ReadOnlyHint))
			}
			if b(ann.DestructiveHint) != tc.destr {
				t.Fatalf("%s: destructive got %v", tc.name, b(ann.DestructiveHint))
			}
			if b(ann.IdempotentHint) != tc.idem {
				t.Fatalf("%s: idempotent got %v", tc.name, b(ann.IdempotentHint))
			}
			if b(ann.OpenWorldHint) != tc.open {
				t.Fatalf("%s: openWorld got %v", tc.name, b(ann.OpenWorldHint))
			}
			if tool.Name != tc.name {
				t.Fatalf("expected name %s, got %s", tc.name, tool.Name)
			}
		})
	}
}
